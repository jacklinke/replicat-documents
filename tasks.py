...
def coverage(c):
    ...
    """
    c.run("coverage run --source replicat-documents runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/replicat-documents.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ replicat_documents")
...
def lint(c):
    ...
    """
    c.run("flake8 replicat-documents tests")
...
def release(c, bumpsize=""):
    ...
    import replicat_documents

    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=replicat_documents.__version__))
    c.run("git push --tags")
